...
This is a walkthrough and an explanation on a simple attack, creating a system service to maintain access after a system reboot
...
- Mimics the official Windows psexec utility.
...
- Ensures the authentication request is routed to the domain's authority rather than attempting to authenticate against local machine accounts.

Administrator
- Targets the user account holding the highest level of privilege.
- Grants unrestricted access to the AD environment.

'Passw0rd!'
- The plaintext secret used to validate authentication via the NTLM Challenge-Response process.

@ <DC_IP>
- The network routing destination.
...
- Combined with create, it acts as an RPC client instructing the SCM to create a new service object.
...
binPath=
- Fills in the image path registry value, defining what executable runs when the service starts.
...
- Establishes persistence, allowing the attacker to maintain access to the Domain Controller even after a system reboot. In this case it is "Backdoor" but in other instances it can be replaced with an actual payload.
...
- The Attack Correlation: This log is tied directly to the persistence phase of the attack, specifically triggered by the `sc create "Maintenance"` portion of the command.
- Event ID 7045 is a Windows log that is generated whenever a new service is installed on the system. It is a primary indicator for security professionals checking if an attacker has established a persistence mechanism.
...
- The Attack Correlation: This log captures the exact sequence of the process executions used to establish the malicious persistence mechanism on the host
- Event ID 1 Overview: The Sysmon log records highly detailed process creation events, providing insight into how an attacker is interacting with the system

Lab Specific Findings:
- Target Host: The malicious activity was logged on the host: `DC-sheehare23.lab.local`.
- Parent/Child: The parent process is `cmd.exe` which spawned the child process `sc.exe`
- Service Creation: Confirms the use of `sc.exe` to create the fake "Maintenance" service. It also shows the malicious `binPath` which used a command shell to write a backdoor string into a new file, guaranteeing the attacker's access survives a system reboot.
...
- Event ID 11 Overview: This is a log that monitors and records file creation operations. This is useful for detecting when attackers download tools, extract malware, or establish persistence mechanisms

**\*This log was in the lab instructions as a Sysmon Log to find however it does not technically exist\***
  - This is due to the fact that while the command was run, it was not executed. This means that without the execution no file will be created and no corresponding log will come as a result 
...
|Attack Step|Command Argument|Corresponding Log|Purpose|
|:----:|:----:|:----:|:----:|
| 1. Authentication | *sc create "maintenance"* | Event ID 7045 | New Service Creation |
| 2. Initial Execution | *sc.exe after cmd.exe* | Event ID 1 | Process of sc.exe after spawn of cmd.exe |
